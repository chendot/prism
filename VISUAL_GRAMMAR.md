...
| `positive` | 正向结果 / 收益 / 增长 | 珊瑚橙 `accent_result` 填充与描边 |
| `negative` | 负向结果 / 成本 / 风险 | 深棕 `surface` 填充 + 珊瑚橙 `accent_risk` 虚线描边 |
| `highlight` | 需要视觉汇聚的核心节点（配合 role=thesis 使用） | 珊瑚橙 `accent_result` 填充 + role 定义的 scale |
...
3. Role 词表本次不重构，但反馈提出的 Actor/Object/Process/Outcome/Decision 拆分方向本身是否认可，作为"未来触发条件"记录下来？比如"当某个 role 在第 3 个不同领域的 ontology 里出现语义冲突时，才启动重构"——需要你定这个触发条件，还是先不写，到时候再说？
